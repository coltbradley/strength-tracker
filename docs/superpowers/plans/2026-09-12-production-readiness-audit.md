...
| --- | --- | --- | --- |
| deployed 2026-09-12 | Coach plan write to PWA Today | Committed as `f789c4e`, published by deploy run 34724741286 (Pages build of `23b916c`). A post-deploy review found the refresh reloads only the selected date, blanking other expanded cards; fixed in the follow-up commit. | Live. |
...
| verified live | Linked Supabase inventory | Re-checked 2026-09-12 after CI deploy: migrations match through `20260907060000`; mcp-server v26, coach v15, push-alerts v5, endurance-sync v3 active. Function secrets carry no `SWEEP_SECRET` and Vault is empty, so the alert sweep runs but delivers nothing. | Configure the sweep per docs/deploy.md step 2. |
