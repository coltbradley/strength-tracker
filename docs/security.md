...
| --------------------------- | --------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| MCP bearer tokens           | `mcp_tokens` (SHA-256 digest only) + each client's config or macOS Keychain | Full MCP tool surface FOR ONE USER: read their training data, write their programs/TMs/goals. Cannot touch `sessions`/`sets` (no tool exists), and cannot reach another user's rows. Only the digest is stored, so a database leak yields no working credential. Revoke with `update mcp_tokens set revoked_at = now()`; re-issue with `scripts/issue-mcp-token.mjs`.                                                                                                                                                                                                   |
| OpenAI tunnel runtime key   | macOS Keychain only, for the Secure MCP Tunnel supervisor                   | Lets the local `tunnel-client` connect the relay to its configured OpenAI tunnel. It cannot call the Strength Tracker MCP endpoint without the separate bearer. Delete the Keychain item and remove the tunnel from OpenAI Platform to revoke it.                                                                                                                                                                                                                                                                                                                       |
...
| VAPID private key           | `push_config` (RLS on, no policies; service role only)                      | Whoever holds it can send a push to every phone subscribed to this deployment: unwanted notifications, not data — a push payload is encrypted to each phone's own key and never carries training data. Generated by the push-alerts function on first use, never set by hand, never logged. A push endpoint (`push_subscriptions.endpoint`) is a capability URL and is never logged either, though alone it is useless: the push service demands a token signed with this key. Rotation is `delete from push_config` plus every device re-enabling the row in Settings. |
...
- `rls_enabled_no_policy` on `push_config` (INFO), since `20260905050000`,
  for the same reason. It holds the VAPID private key; only the push-alerts
  function reads it, as the service role. `scripts/validate-db.mjs` asserts
  that an authenticated user reads zero rows from it and cannot insert one.
